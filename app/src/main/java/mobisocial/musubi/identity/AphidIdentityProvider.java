...
public class AphidIdentityProvider implements IdentityProvider {
	...
	public boolean initiateTwoPhaseClaim(IBIdentity ident, String key, int requestId) {
        ...
        try {
            ...
            /*
             * by haoyuheng
             *  just return 200 instead of get from server
             */
...
//        	    TwilioService.sendSMS(phoneAccount, pToken);
	...
	private byte[] getAphidResultForIdentity(IBIdentity ident, String property)
			throws IdentityProviderException {
		Log.d(TAG, "Getting key for " + ident.principal_ + " "+property);
		...
		try {
			...
			/*
			 * by haoyuheng
			 * use JsonGernerator instead of getAphidResult from server
			 */
			
			//JSONObject resultObj = getAphidResult(userinfo);
			
			JSONObject resultObj = JsonGernerator.GerneratorAphidResult(ident.temporalFrame_);
